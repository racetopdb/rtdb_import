...
struct thread_param_insert_table_v2_t
{
    thread_param_insert_table_v2_t()
        : lines()
    {}
...
static bool next_bool( thread_param_insert_table_v2_t * param, size_t table_i, uint64_t insert_line_index )
{
    uint64_t x = (uint64_t)calc_start_hash( table_i ) + insert_line_index;
    return 0 == ( x % 2 );
}

static int next_int( thread_param_insert_table_v2_t * param, size_t table_i, uint64_t insert_line_index )
{
    uint64_t x = (uint64_t)calc_start_hash( table_i ) + insert_line_index;
...
static float next_float( thread_param_insert_table_v2_t * param, size_t table_i, uint64_t insert_line_index )
{
    uint64_t x = (uint64_t)calc_start_hash( table_i ) + insert_line_index;
...
void * insert_table_thread_v2( void * _param )
{
    thread_param_insert_table_v2_t * param = (thread_param_insert_table_v2_t *)_param;
...
int insert_table_v2( int argc, char ** argv )
{
    ...
    // 测试代码  
    {
        const char* line = "A,2021/5/1 6:04,2021/5/1 6:04,0,,,,,,,,,,;1006;1010;,ZKC02,\"{\"\"\"\"LIMIT_RPM_STATUS\"\"\"\":{\"\"\"\"lastTime\"\"\"\":1619820252000,\"\"\"\"value\"\"\"\":\"\"\"\"NO_EXECUTE\"\"\"\"},\"\"\"\"ENGINE_TYPE\"\"\"\":{\"\"\"\"lastTime\"\"\"\":1619820252000,\"\"\"\"value\"\"\"\":\"\"\"\"5\"\"\"\"},\"\"\"\"TAMPER_STATUS\"\"\"\":{\"\"\"\"lastTime\"\"\"\":1619820252000,\"\"\"\"value\"\"\"\":\"\"\"\"NO_EXECUTE\"\"\"\"}}\"";

        int  line_len = (int)strlen(line);
        const char* sep = ",";
        std::vector<tsdb_str> vt_data;
        std::vector<BOOL> data_is_string;
           
        int data_count = 0;
        p->tools->csv_line_to_array2(line, line_len, sep, NULL, NULL, &data_count);

        vt_data.resize(data_count);
        data_is_string.resize(data_count);

        p->tools->csv_line_to_array2(line, line_len, sep, &vt_data[0], &data_is_string[0], &data_count);

        std::string s (vt_data[data_count - 1].ptr, vt_data[data_count - 1].len);
        TSDB_INFO(p, "%s", s.data());
    }
#endif
    // The 'tools' structure contains many functions that we prepared for you.
    // you will see a lot of code call these functions via the 'tools'.
    ...
    std::vector< thread_param_insert_table_v2_t >   threads;
    threads.resize( thread_count );
    for ( size_t i = 0; i < threads.size(); ++ i ) {
        thread_param_insert_table_v2_t & item   = threads[ i ];
        item.engine             = engine;
    ...
    for ( size_t i = 0; i < threads.size(); ++ i ) {
        thread_param_insert_table_v2_t & item = threads[ i ];
        item.exited = false;
        if ( unlikely( ! p->tools->thread_start( & item.thread, insert_table_thread_v2, & item, 0 ) ) ) {
